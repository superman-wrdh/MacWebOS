...
import { Maximize, Minus, Plus, Trash2, Link, Check, PlusCircle } from 'lucide-react';
    ...
    <div className="p-6 flex flex-col gap-6 bg-white/80 backdrop-blur-md h-full overflow-y-auto">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3 border-b border-gray-200 pb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-800">模型资源管理</h2>
            <p className="text-xs text-gray-500">管理并切换不同的 Live2D 模型 URL</p>
          </div>
        </div>
        ...
        <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-3">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider">添加新模型</h3>
          <div className="space-y-2">
            <input 
              type="text" 
              placeholder="模型名称 (例如: 希儿)" 
              value={newModelName}
            ...
            <input 
              type="text" 
              placeholder="模型 JSON URL (model.json)" 
              value={newModelUrl}
      ...
      <div className="mt-auto p-3 bg-indigo-50 rounded-lg border border-indigo-100">
        <p className="text-[11px] text-indigo-700 leading-relaxed">
          <strong>提示：</strong> 你可以从网络上获取 Live2D 模型的 <code>model.json</code> 链接并粘贴到此处。切换模型后，桌面上的模型会自动重新加载。
        </p>
      </div>
    </div>
  );
